...
- (void)collect 
{
    ...
    CFTimeInterval startTimeFortotalDiskSpace = CACurrentMediaTime();
    NSUInteger totalDiskSpace = (NSUInteger)(([[self totalDiskSpace] longLongValue]) / KSystemCollectorDiskDivisor);
    CFTimeInterval elapsedTimeFortotalDiskSpace = CACurrentMediaTime() - startTimeFortotalDiskSpace;
    ...
    [self addDataPoint:[NSNumber numberWithUnsignedInteger:totalDiskSpace] forKey:KDataCollectorPostKeyTotalDisk];
    [self addDataPoint:[NSString stringWithFormat:@"%f",elapsedTimeFortotalDiskSpace*1000.0] forKey:KDataCollectorPostKeyTotalDiskElapsedTime];
    ...
    KCOLLECTOR_DEBUG_MESSAGE(@"Total Disk Space: %ld", (unsigned long)totalDiskSpace);
